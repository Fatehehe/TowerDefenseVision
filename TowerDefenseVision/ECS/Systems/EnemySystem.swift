...
public struct EnemySystem: System {
    ...
    // Must not run concurrently with PortalSystem (spawns EnemyComponent
    // entities) or CombatSystem (removes them on a hit) since all three
    // touch the same enemy entities.
    public static var dependencies: [SystemDependency] {
        [.after(PortalSystem.self), .after(CombatSystem.self)]
    }
